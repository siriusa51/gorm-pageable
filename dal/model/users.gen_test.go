...
func init() {
	...
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&types.User{}) fail: %s", err)
	}
}
...
func Test_userQuery(t *testing.T) {
	...
	err = _do.Create(&types.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.Save(&types.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.CreateInBatches([]*types.User{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*types.User{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&types.User{}, 0, 1)
	if err != nil {
